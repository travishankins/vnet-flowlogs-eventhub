...
This repository provides **two sample implementation approaches** for processing VNet flow logs and forwarding them to an Event Hub:
...
The Function handler has unit coverage for success, retryable failure, and Event
Grid subscription validation. Neither implementation has been certified for a
particular production workload. The Function requires a function key: configure
the Event Grid webhook URL with its `code` query parameter and protect that URL.

## Verification

Install `function-app/requirements.txt`, then run
`python -m unittest discover -s tests -v`. CI does not contact Azure.
Before release, test actual flow-log arrival, host function discovery, Event Grid
subscription validation, RBAC, and Event Hub receipt in a nonproduction environment.

## Limitations

Delivery is at least once: a retried batch can resend already processed events.
Consumers must deduplicate. A blob is sent as one Event Hub message; oversized
blobs fail and require a record-splitting design before high-volume use.
Configure Event Grid dead-lettering and monitor failures. The Logic App path has
not gained the Function handler's regression coverage.

## Release and Rollback

Publish only to a confirmed Function App. Update the webhook URL to include a
function key before switching traffic. Retain the previous artifact and event
subscription configuration; do not restore anonymous access as a rollback.
...
│   ├── flowlogs/function.json           # Function configuration and entrypoint
...
This project is licensed under the [MIT license](LICENSE) and provided as-is.
